...
BEGIN
	BEGIN TRY
		BEGIN TRAN
			IF NOT EXISTS (SELECT * FROM DOHOTPHIM WHERE MaDoHot=@MaDoHot)
				RAISERROR(N'Không tìm thấy mã độ hot', 16, 1)

			UPDATE DOHOTPHIM
			SET TenDoHot=ISNULL(@TenDoHot, TenDoHot),
